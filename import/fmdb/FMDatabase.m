...
#ifdef DEBUG
static const NSTimeInterval kTimeoutInterval = 0.2f;
#define YoyoDetectTimeoutBegin      \
    NSDate* beginDate = [NSDate date]

#define YoyoDetectTimeoutEnd       \
    NSDate* endData = [NSDate date];    \
NSTimeInterval interval = [endData timeIntervalSinceDate:beginDate];    \
if (interval > kTimeoutInterval) {   \
    NSLog(@"%s %d execute query timeout(%@ > %@), sql = %@ ",   \
    __FUNCTION__, __LINE__, @(interval), @(kTimeoutInterval), sql);  \
}
#else
#define YoyoDetectTimeoutBegin
#define YoyoDetectTimeoutEnd
