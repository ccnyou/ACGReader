...
@property(nonatomic, assign) BOOL needReloadOnAppear;
@end
...
- (void)_loadCachedNodes {
    ...
    for (LinkNode *node in nodes) {
        if (node.cacheState == ACGCacheStateRunning) {
            // 恢复出来发现上次任务进行中
...
- (void)_parseAcgListByUrl:(NSString *)url {
    NSError *error = nil;
    NSString *htmlString = [NSString acg_stringWithContentsOfURL:[NSURL URLWithString:url]
    ...
    if (error) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSString *msg = [NSString stringWithFormat:@"刷新失败: %@", @(error.code)];
...
- (NSString *)_parsePreviewImageUrl:(OCGumboElement *)rootNode aNode:(OCGumboNode *)aNode {
    ...
    NSUInteger beginPos = [mouseOver rangeOfString:@"("].location;
    NSUInteger endPos = [mouseOver rangeOfString:@")"].location;
...
- (void)_showLocalImageBrowser:(LinkNode *)node {
    ...
    ImageBrowserViewController *vc = [ImageBrowserViewController show:self type:PhotoBrowserVCTypePush index:(NSUInteger)node.readingIndex imageUrls:imageUrls];
