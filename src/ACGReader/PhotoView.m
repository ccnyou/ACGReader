    ...
    if (self) {
        ...
        [manager cachedImageExistsForURL:[NSURL URLWithString:photoUrl] completion:^(BOOL isInCache) {
            if (!isInCache) {
                ...
                [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl]
                                  placeholderImage:nil
                                         ...
                                         completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                                             self.imageView.frame = [self calculateOriginImageSizeWith:image];
                                             NSLog(@"图片加载完成");
                                             if (!isInCache) {
                ...
                self.imageView.frame = [self calculateOriginImageSizeWith:cachedImage];
...
- (void)scrollViewDidZoom:(UIScrollView *)scrollView {
    CGFloat offsetX = ((_scrollView.bounds.size.width > _scrollView.contentSize.width) ? (_scrollView.bounds.size.width - _scrollView.contentSize.width) * 0.5f : 0.0f);
    CGFloat offsetY = ((_scrollView.bounds.size.height > _scrollView.contentSize.height) ?
            (_scrollView.bounds.size.height - _scrollView.contentSize.height) * 0.5f : 0.0f);
    self.imageView.center = CGPointMake((CGFloat)(_scrollView.contentSize.width * 0.5f + offsetX), (_scrollView.contentSize.height * 0.5f + offsetY));
}

//2.重新确定缩放完后的缩放倍数
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(CGFloat)scale {
    [scrollView setZoomScale:(CGFloat)(scale + 0.01) animated:NO];
...
- (CGRect)calculateOriginImageSizeWith:(UIImage *)image {
    CGFloat originImageHeight = [self imageCompressForWidth:image targetWidth:SCREEN_WIDTH].size.height;
    if (originImageHeight >= SCREEN_HEIGHT) {
        originImageHeight = SCREEN_HEIGHT;
    }

    CGRect frame = CGRectMake(0, (SCREEN_HEIGHT - originImageHeight) * 0.5f, SCREEN_WIDTH, originImageHeight);
